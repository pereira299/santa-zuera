...
export async function GET(req: NextRequest) {
  ...
  const id = (lastStoredEpisode[0].fields.countNumber as number) +1;
  const youtube = new GoogleYoutube();
  const channelId = process.env.YOUTUBE_CHANNEL_ID;
  const youtubeEpisodes = await youtube.search(`Santa Zuera`, ["video"], channelId);

  return NextResponse.json({
    message: "Sync",
    id: id,
    data: youtubeEpisodes.items,
  });
